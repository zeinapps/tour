...
function exray_theme_page($active_tab = '') {
    ...
    <div class="wrap">
        ...
        <h2 class="nav-tab-wrapper">
            ...
            <a href="?page=exray_theme_options&tab=general_options" class="nav-tab  <?php echo $active_tab == 'general_options' ? 'nav-tab-active' : ''; ?>"><?php _e('General Options', 'exray'); ?></a>

            <a href="?page=exray_theme_options&tab=custom_css" class="nav-tab  <?php echo $active_tab == 'custom_css' ? 'nav-tab-active' : ''; ?>"><?php _e('Custom CSS', 'exray'); ?></a>
...
function exray_theme_general_options_section_callback() {
    _e('You can modify Theme backend feature here. Looking to modify Theme Frontend / Visual style? Please check', 'exray');
    ?>
...
function exray_menu() {
    $menus= array(
        'top-menu' => array(
            'name' => 'top_menu',
            'label' => __('Hide top menu', 'exray')
        ),
        'main-menu' => array(
            'name' => 'main_menu',
            'label' => __('Hide main menu', 'exray')
        ),
    );
...
?>
    ...
    &nbsp;
    <label for="exray_theme_general_options[go_to_top_navigation]" class="description"><?php _e('Hide go to top link on footer', 'exray'); ?></label>
...
function exray_theme_validate_general_options($input) {
    ...
    if ($valid['contact_form_email_receiver'] != $input['contact_form_email_receiver']) {
        add_settings_error(
            ...
            __('Invalid email, please fix', 'exray'), 
            'error'
        );
    }   
...
*/', 'exray');
    ...
    ?>
    <h3><?php _e('Custom CSS', 'exray'); ?></h3>
    <p><?php _e('Add your custom CSS below. For your information, Custom css below will override colors from', 'exray'); ?> <a href="<?php echo admin_url('customize.php'); ?>" title="Theme Customizer">Theme Customizer</a></p>
    <textarea id="custom_css_textarea" name="exray_custom_css" style="height:300px;" placeholder="<?php echo _e('/*** Add your custom CSS here. Do not edit style.css directly. ***/', 'exray'); ?>"><?php echo esc_textarea($exray_custom_css_options); ?></textarea>
    <?php
}
